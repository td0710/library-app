...
export const Loans = () => {
  ...
  useEffect(() => {
    ...
    window.scrollTo(0, 0);
  }, [authState, checkout, tmp]);

  if (isLoadingUserLoans) return <SpinnerLoading></SpinnerLoading>;
  if (httpError) {
    return (
  ...
  const returnBook = async function (bookId: number) {
  ...
  const renewLoan = async function (bookId: number) {
          ...
          <>
            ...
            {shelfCurrentLoans.map((shelfCurrentLoan) => (
              <div key={shelfCurrentLoan.book.id}>
                ...
                <div className="card d-flex mt-5 mb-5">
